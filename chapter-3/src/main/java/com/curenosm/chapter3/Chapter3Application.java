...
public class Chapter3Application {
  ...
  public static boolean isPalindromeFunctional (String str) {
    ...
    return forward
      .toString()
      .equals(new StringBuilder(forward).reverse().toString());
  }
  ...
  public ApplicationRunner countingElements () {
    return args -> {
      ...
      count = Stream.of(1, 2, 5, 21, 1, 0, 21, 2)
        .collect(Collectors.counting()); // Downstream collectors for downstream post-processing
  ...
  public ApplicationRunner summaryStatistics () {
    return args -> {
      DoubleSummaryStatistics stats = DoubleStream.generate(Math::random)
        .limit(1_000_000) // Java 7+ allow underscore for numeric literals
        .summaryStatistics();
  ...
  public ApplicationRunner findingElements () {
    return args -> {
      Optional<Integer> firstEven = Stream.of(3, 1, 2, 3, 5, 7, 8).filter(n -> n % 2 == 0)
        .findFirst();
      ...
      Optional<Integer> sequential = Stream.of(3, 1, 4, 1, 5, 9, 2, 6, 5)
        .unordered()
        .map(this::delay)
        .findAny();
  ...
  public ApplicationRunner mapAndFlatMap() {
    return args -> {
      ...
      c1.addOrder(new Order(1))
        .addOrder(new Order(2))
        .addOrder(new Order(2));

      c2.addOrder(new Order(4))
